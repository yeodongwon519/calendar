...
// 큰 위젯: 2주치(이번 주+다음 주) 7칸×2줄 그리드, 각 칸에 그날 할일.
// 중간 위젯: 이번 주 1줄. 작은 위젯: 오늘 목록.
(async function () {
  var C = {
  ...
  // ----- 한 주(7칸) 그리드 한 줄 만들기 -----
  function buildWeek(weekStart, maxPerCol, colW, rowH) {
    var grid = w.addStack();
  ...
  if (fam === "large") {
    // 2주치: 이번 주 + 다음 주
    buildWeek(weekStart, 7, 45, 148);
    w.addSpacer(6);
    var next = new Date(weekStart); next.setDate(weekStart.getDate() + 7);
    buildWeek(next, 7, 45, 148);
  } else {
    // 중간: 이번 주 한 줄
    buildWeek(weekStart, 8, 45, 150);
  }
